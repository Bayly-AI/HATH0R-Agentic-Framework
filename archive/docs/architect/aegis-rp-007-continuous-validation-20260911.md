...
Today those are enforced by **human-readable checklists** (Infra AI Agent Linting Guide) and **batch Q-gates at the end**. Agents can drift for dozens of tool calls before anything refuses. That violates AEGIS canonical rule 2 (*refuse-never-guess*) and rule 5 (*builder + verifier*) for the bulk of agent wall-clock time.
...
- never becomes a second control plane (still only through `aegis`),
...
```json
{
  ...
  "remediation": "Remove secret; rotate if committed; use aegis secrets exec",
  "subject": {
...
- Ledger lives at `.aegis/state/runs/<run_id>/assumptions.jsonl` (UPL); purged with run retention policy.
...
| `pr.ready` | `suite.claim.pr-ready` | `aegis delivery pr prepare` / open |
...
|---|---|---|---|
| `fs.write` / staged diff | CLI hook, editor bridge, `aegis delivery diff watch` | `suite.change.fast` | Changed paths only |
| `git.stage` / `git.commit` prep | `aegis delivery commit` | `suite.change.default` | Staged set |
...
| `claim.assert` | Agent or `aegis validate claim` | claim suite | Declared claim |
...
`aegis validate watch --run <id>` is a **resident helper** (like Observation drain), not a bot public listener:
...
### 4.1 Platform / AEGIS structural linters (always on)

| ID | Class | Detects | Severity |
|---|---|---|---|
| `ml-upl-layout` | `upl.layout` | Missing/invalid `.aegis/`, forbidden parallel control plane in `bin/` | block |
...
Projects register language micro-linters in `.aegis/rules/linters.yaml` (wrappers over ruff/eslint/phpcs/etc.) with:
...
Infra AI Agent Linting rules (forEach, node: imports, no `any`, complexity caps, JSDoc) land here as **project rule packs**, not hard-coded into AEGIS core. Core only requires that *some* pack is bound for languages present in the repo (`ml-langpack-present`).
...
`selftest` for a micro-linter = golden fixtures under `.aegis/rules/fixtures/<class>/`.
...
- `aegis delivery commit|pr prepare` → mandatory `validate claim` preflight
- `aegis process …` step boundaries → optional/required per runbook metadata `validation.suite`
- `aegis knowledge push` → knowledge validators
- `aegis bots selftest` → includes validator bots + micro-linter fixtures
- `aegis doctor validation` → fabric health (linter pack present, watch status, ledger path)

Global agent orientation pack (NFR-002) gains **one** line-level pointer, not full suite dump:

```text
aegis validate suite list --fields id,tier,blocking --limit 10
```
...
3. After each logical edit batch, run `aegis validate change --diff worktree --tier L1` (or rely on watch).
4. Before any "done" / PR / build claim, run the matching `aegis validate claim …` and treat non-zero as incomplete work.
...
Suites are data in `.aegis/rules/suites/*.yaml`, signed when distributed org-wide (same provenance model as manifests).
...
**AC:** `aegis validate --help` lists change/assumption/claim/suite/watch; ARCH diagram documents four immune altitudes.
...
Project language rules MUST register as linter packs; the advisory-only AI Agent Linting Guide is non-authoritative under AEGIS.
...
Tier latency budgets in §3.2 MUST be measured by benchmark-Bot and surfaced in `aegis doctor validation`.
...
**AC:** Toggle in `.aegis/rules/validation.yaml`; default off for v1, on for platform dogfood.
...
sequenceDiagram
  ...
  participant V as aegis validate
...
| Q-gates report JSON under `.infraOS/reports` | **Adopt shaped → `.aegis/state/runs/*/validation/`** | Same evidence idea, UPL-canonical |
...
Dogfood target: AEGIS itself + one Infra product (InfraAPI) as first pack hosts.
...
**Command-tree fit:** Prefer top-level `aegis validate` (high agent traffic). If the ≤8 domain cap binds, fold as `aegis proctor validate …` (since Proctor owns gates). Recommendation: **top-level `validate`** — agent discoverability beats pedantic cap; adjust PLAT-008 to "≤ 9" or move `bots` under `tower`. *(Accepted by ADR-003 §2.2, 2026-09-13: both measures adopted — `validate` top-level, `bots` → `tower`, cap ≤ 9.)*
