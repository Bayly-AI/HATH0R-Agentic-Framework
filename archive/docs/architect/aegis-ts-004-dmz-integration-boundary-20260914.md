...
|---|---|---|---|
| **TS4-D-001** | **Built in Go inside the single `aegis` binary.** New packages under `internal/policy/*`; enforcement inserted into the existing `proctor.Admit` seam (TS-002 §7) and TS-003 rules engine. No daemon, no second evaluator. | One authoritative chokepoint (`BOT-TAX-006`, `AEG-GW-003`); NFR-001 cold-start budget. | Language per TS-001 §16; seam is not. |
| **TS4-D-002** | **Project rules are data with the bot-rule schema.** `.aegis/rules/rules.yaml` uses the TS-003 `rules-1.0.0` predicate schema with two deltas: `scope: project` and `applies_to` expressed as **capability patterns**, never bot names. Effects remain `refuse \| degrade` only. Evaluated at admission **after platform gates and before bot pre-rules**. | Fills the empty middle of `AEG-BOT-GOV-008`'s chain without a second rule language; narrowing-only by construction (`AEG-BOT-GOV-003`); capability addressing per `AEG-MAN-003`. | No. |
...
| **TS4-D-008** | **Nested `.aegis/` roots: nearest wins for config, outer-first narrowing for governance, root owns the run.** *Provisional* — implementation blocked on the report-G7 ADR. | Extends the add-never-remove rule from bot↔family to child↔parent project. | Yes, by the ADR. |
| **TS4-D-009** | **No new top-level domain; no new event type.** Surface folds under `aegis repo policy …` and the existing `aegis proctor gateway explain`. Conflicts ride `validation.finding` and `contract.refused`. | `AEG-REQ-PLAT-008` (≤ 9), CANON-001 §3 stability. | No. |
...
Everything runs as short-lived CLI invocations over durable `.aegis/` state (TS-001 §2.1). The DMZ adds no process; it adds a **static admission pass** (outer wall) that runs at `repo validate`, install, and suite/graph load, and a **dynamic admission step** (inner wall) inside the existing `proctor.Admit`.
...
aegis/internal/
  ...
  cli/repo/policy/      # aegis repo policy show|validate|explain|floors
schemas/
...
machine tier:
  ${XDG_CONFIG_HOME}/aegis/policy.yaml               # may raise profile / add floors; never lower
  ${XDG_STATE_HOME:-~/.local/state}/aegis/policy/pack.json   # cached Tower policy pack (TUF-verified, TTL-stamped)
```
...
|---|---|---|
| `aegis repo validate` (always; `--policy` adds the merged-policy report) | whole `.aegis/rules/**` + `AGENTS.md` chain | yes, per profile |
| `aegis tower bots install <ref>` | bundle only (TS-003 §7.2) — unchanged; DMZ adds nothing here | n/a |
...
| `aegis process run open` | effective profile + policy digest stamped into `run.json` | profile-downgrade attempt refuses |
      ...
      schema:
        properties:
          effects:
            properties:
              paths_written:
                items: { pattern: "^(src/|test/|\\.aegis/state/)" }
    on_violation: refuse                        # refuse | degrade — NO grant vocabulary
    message: "Writes outside src/, test/, or .aegis/state/ are not permitted in this project."
    remediation: "Confine writes to the declared scopes; request a project-rule change via ticket."
    fixtures: .aegis/rules/fixtures/rules/prj-r-001/   # ≥ 1 passing + ≥ 1 violating
```
...
import gov "github.com/baylyai/aegis/internal/governance/rules" // TS-003; module path pending M0 freeze
...
type File struct {
    ...
    Root    string     `json:"-"`     // which .aegis/ root supplied it (nested chain)
}
...
Project-rule fixtures live at `.aegis/rules/fixtures/rules/<rule-id>/` with the same shape as TS-003 bundle fixtures (`≥ 1` passing, `≥ 1` violating context per rule). They are exercised by `aegis repo policy validate` and by `suite.health.project`; a rule lacking either fixture kind fails `Compile` (parity with the `AEG-BOT-LIF-003` registration refusals). Fixtures are evaluated platform-side against the §5.3 context, so a Go bot and a Python bot dispatched under the same project rule are refused identically (`AEG-BOT-ANA-004`). There is no project `selftest`; `repo policy validate` is the project-side analogue.
...
min_profile:                       # per project key; absent or report-only = no minimum
  ...
  "AEGIS-CORE": enforced           # example: the platform's own repo is pinned enforced
floors:
  suites:
...
```text
Discover(cwd) -> roots[]:  every ancestor directory containing .aegis/, ordered repoRoot → nearest
...
Run owner:  repoRoot .aegis/state/runs/ ; nested roots may not open runs
```
...
patterns:
  - id: ac-001
    ...
    remediation: "Reword to reference `aegis delivery pr prepare`; branch/PR must carry the ticket key."
  - id: ac-002
...
Org-distributed packs are signed (`AEG-THR-004`); the bundled pack is `go:embed`. Projects MAY extend with `.aegis/rules/agents-conflict.yaml` (additive; cannot remove bundled patterns).
...
- Suppression: an HTML comment `<!-- aegis:suppress ac-001 reason="…" ticket="KEY-123" -->` on the preceding line downgrades that hit to `info` and emits an `info` finding recording the suppression (waivers are events, never silence). Missing `ticket` → suppression ignored. The token is deliberately `suppress`, not `allow`: it suppresses a *finding* about prose; it grants nothing at any gate.
...
```json
{
  ...
  "details": {
    "conflict_class": "C2",
    "sides": [
      { "role": "app",       "artifact": ".aegis/rules/suites/suite.change.default.yaml", "ref": "blocking_classes", "digest": "sha256:…" },
    ...
    "next_required": ["edit .aegis/rules/suites/suite.change.default.yaml: add sec.secrets_in_diff", "aegis repo policy validate"],
...
```yaml
# .aegis/rules/validation.yaml (TS-001 §13 keys unchanged; `profile` block added)
profile:
...
An undeclared `profile.active` resolves to `dev`; `aegis repo init` writes `report-only` explicitly so Phase 0 (§15) is honest on disk. The effective profile is stamped into `run.json`, every finding's `provenance.profile`, and the rule context `env.profile`. A change of effective profile between two invocations on the same run is recorded as a `degraded` condition on the run (no new event type).
...
5. **Steer**: `aegis proctor gateway explain` and `aegis repo policy explain <id>` render `sides[]` and `next_required[]` within the NFR-002 budget.
...
| **Configuration (nearest wins)** | `--flag` → project `.aegis/rules/*.yaml` (nearest root) → machine → embedded | tunables, tiers, thresholds, suite/graph *content* | **bounded below by floors** (this spec) and by `config_schema`; config tunes, never governs (RP-014 §3.6) |
...
|---|---|---|---|
| 1 | `aegis repo init` (or `--from-infraos`) → `.aegis/` skeleton with `profile.active: report-only`, empty `rules.yaml`, bundled floors | P0 | — |
| 2 | `aegis repo validate --policy` → layout, chain, floors, C8 parallel-control-plane findings | P0 | C8, layout drift |
...
| 7 | derive/author `graph.yaml` for the primary runbook; `aegis process graph validate` | P2 | C4 |
...
Project rules are evaluated platform-side at `Admit`; moving a bot out-of-proc changes nothing here. The effects wrapper (`aegis.bot-response/1`, TS-003 §6) already supplies what project post-rules need.
...
| **A2** subverted local process | edits `.aegis/rules/*` to *add* `refuse` rules (self-denial, harmless) or to *widen* (floors refuse); tampers the policy cache | floors are org data (Tower/embedded), never project-writable; cache is digest-keyed and disposable (§2.3); `policy_digest` stamped on every run for Tower cross-check |
...
| RP-014 §3.4 item 2 | names `.aegis/rules/rules.yaml` as the project-rules artifact | proposed (D8 amendment — operator decision) |
| CORE §7.1 | `rules/rules.yaml` shown in the UPL tree | proposed |
| ADR (new) | nested `.aegis/` roots (report G7) | required before P4 |
