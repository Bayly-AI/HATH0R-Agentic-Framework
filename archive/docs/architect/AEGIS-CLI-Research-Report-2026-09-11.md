# AEGIS CLI Research Report
**Subject:** AEGIS CLI Vision vs InfraOS CLI (`infraos-os`) vs Industry Practice
...
## 1. What the AEGIS outline is saying
...
> "The AEGIS CLI is the single point of contact for the agent to interface with the computer, code, assets and knowledge."
...
AEGIS assumes a standard, repo-agnostic layout so the CLI and bots can reason about every project identically:

```text
<project>/
  .ai/aegis/ # hidden AEGIS metadata, manifests, and rules
...
The outline also requires a local `AGENTS.md` in each significant folder, with parent files pointing to child scopes, so agents load relevant instructions without one monolithic context file. Every AEGIS project is a Git project, and the Makefile is a thin façade over stable CLI contracts. The exact capitalization and migration from existing `.infraOS` assets remain implementation decisions; the list above preserves the outline labels.
...
InfraOS already implements large parts of the AEGIS model:

| AEGIS concept | InfraOS reality | Evidence |
|---------------|-----------------|----------|
...
**Verdict:** Conceptually, InfraOS is already an AEGIS-class platform CLI — not a thin wrapper around scripts. The AEGIS diagram is less a "new product" and more a coherent packaging of what the platform already believes.

### 2.2 Structural gaps vs the diagram

| AEGIS expectation | Gap in current CLI |
|-------------------|--------------------|
...
| Universal Project Layout (`.ai/aegis`, CFG/SRC/LIB/BIN/DIST/Test/docs/`.docker`, scoped AGENTS files) | Product repos are Makefile/AGENTS.md-driven, but the target folder contract and per-folder instruction scoping are not uniformly enforced by the CLI. |
...
5. **Incomplete hierarchy productization** — playbooks/runbooks/checklists are catalog/search/show; workflows is the only real executor. AEGIS wants runbooks to *own* workflow execution and checklists to *track* completion as one lifecycle.
...
8. **Consumer repo friction** — Communications teaches Makefile as the primary lifecycle; AEGIS wants CLI as the single contact with Make as a thin façade.
...
| Source | Key takeaways relevant to AEGIS/InfraOS |
|--------|------------------------------------------|
...
**InfraOS implication:** Keeping Connections + MCP separate (AD-005) is right. The missing piece is an **operations/intent façade** (AEGIS Procedure/Strategy/Runbook) so agents don't navigate 79 top-level commands.
...
This maps almost 1:1 to AEGIS **Proctor-Bot** plus the existing TTHC governance rules (development-only writes, no agent deploy, DVO deploy-ticket handoff).
...
```mermaid
flowchart TB
  subgraph aegis [AEGIS Target Model]
...
**Delta:** AEGIS specifies **roles + chained hierarchy + machine contract + knowledge lifecycle + project-layout contract**; InfraOS exposes capabilities as a wide flat menu without enforcing all of those relationships.
...
**A. Productize the AEGIS hierarchy as a runtime, not only catalogs.**
...
## 6. Mapping AEGIS "CLI as control plane" to existing governance rules

The Communications AGENTS rules already encode control-plane law that AEGIS would place in Proctor/Operator:
...
| Dimension | Score vs AEGIS vision | Notes |
|-----------|----------------------|-------|
...
- Submitted AEGIS outline images: [`images/00-outline-agent-anatomy-taxonomy.png`](./images/00-outline-agent-anatomy-taxonomy.png), [`images/01-outline-universal-project-layout.png`](./images/01-outline-universal-project-layout.png), [`images/02-outline-universal-project-layout-detailed.png`](./images/02-outline-universal-project-layout-detailed.png), [`images/03-outline-micro-bot-architecture.png`](./images/03-outline-micro-bot-architecture.png)
