...
```yaml
# .aegis/knowledge/<topic_key>/<record_id>.md — YAML frontmatter + markdown body
...
|---|---|---|---|
| **Project** | `.aegis/knowledge/**` (files in git) | `.aegis/state/cache/knowledge.db` (SQLite: FTS5 + vector table) | CLI direct + project MCP (read surface) |
| **Machine** | `${XDG_DATA_HOME:-~/.local/share}/aegis/knowledge/**` | machine `knowledge.db` (same schema) | CLI direct |
...
Design rules: the SQLite index is disposable and rebuilt from files (`aegis knowledge index refresh`); embeddings are optional per machine — **FTS5 keyword search MUST work with zero model dependencies**, vectors improve ranking when a local embedding service is configured (Class B container locally, per the containerization taxonomy).
...
- **τ (confidence threshold)** is configuration, not folklore: `.aegis/rules/knowledge.yaml` `thresholds: {project: τ₁, machine: τ₂, organization: τ₃}` with bundled defaults; calibrated per org from retrieval-feedback telemetry (`knowledge.retrieval` events — registry addition, CANON-001 §3).
