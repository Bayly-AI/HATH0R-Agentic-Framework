...
HATHOR already has two of the three fabrics it needs, but not the third:
...
- **RP-007 §6.1** makes validation the *agent's* responsibility ("Agents MUST … run `hath0r validate change` … `hath0r validate claim` …"). This is **voluntary compliance**.
...
HATHOR has *already solved* false completion mechanically (RP-007/TS-001). This decision must additionally solve **omission** and **order violation** — and do so system-natively.
...
- **Single control plane:** ✓ No new surface; reached via `hath0r process`/`hath0r proctor`. **No bus.**
...
**HATHOR adopts event-triggered central orchestration with an enforcing gateway (Option D)** as the coordination model for all bot and linter execution:
