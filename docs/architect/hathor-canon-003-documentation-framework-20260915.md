...
The HATHOR/HATHOR corpus already practices strong conventions — stable Document IDs, a status front-matter block, an INDEX with a Doc-ID column, and a `PENDING-EDITS` change-control register (CANON-001 §7.5; INDEX.md). What it does **not** yet have is a single, machine-checkable contract that:
...
These are the HATHOR corpus types; membership lives here (extends CANON-001 §7.1). A new type requires an amendment to this section.
...
- **New documents** use `HATHOR-<TYPE>-<NNN>`, `TYPE ∈ {ADR, RP, TS, PLAN, REQ, ARCH, CANON, GUIDE, REPORT, SESSION}` (adds `GUIDE`, `REPORT`, and the registered `SESSION` type to the CANON-001 set), `NNN` zero-padded and monotonic per type.
...
`hath0r-<type>-<nnn>-<slug>-<yyyymmdd>.md`, lowercase, hyphenated. The `<yyyymmdd>` records the file's creation/authored date and does **not** change on later edits (edits are tracked by `updated`/`version`, not the filename). Existing files keep their names; the INDEX and front-matter `id` are the lookup keys.
...
`AEG-DOC-007`: the corpus MUST expose staleness both per-document (`stale`) and in aggregate (index + `hath0r`-style report), so drift is visible rather than silent.
...
```
# HATHOR / HATHOR Architecture Corpus
...
## Canon
- [HATHOR-CANON-010 Registries](./hathor-canon-001-registries-20260913.md): gates, events, refusal codes, linters, bot roster, IDs.
- [HATHOR-CANON-012 Documentation Framework](./hathor-canon-003-documentation-framework-20260915.md): how every doc is structured, versioned, reviewed, and indexed.

## Decisions
- [HATHOR-ADR-003 Greenfield command surface](./hathor-adr-003-greenfield-command-surface-20260913.md): nine-domain CLI.
```
