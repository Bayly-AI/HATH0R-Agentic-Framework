---
id: HATHOR-RP-003
title: HATHOR Research Paper 003 — Telemetry Transport
...
sources: []
---
# HATHOR Research Paper 003 — Telemetry Transport
...
- Cost: a new mandatory daemon on every HATHOR machine — a second control plane beside the CLI, its own health/upgrade/port surface (and port-registry governance burden), and a hard dependency for every bot run. For per-run, low-rate machine telemetry this is capacity we don't need at the machine tier. **Rejected for v1 machine tier.**
...
2. CLI stamps the event (see §4), appends to the active spool segment under the project spool (`.hath0r/state/spool/<date>/<segment>.jsonl`; non-project fallback and machine indexing per ADR-004 §2.2).
