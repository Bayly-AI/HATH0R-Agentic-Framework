---
id: HATHOR-RP-004
title: HATHOR Research Paper 004 — Draft→Verified Knowledge Promotion Workflow
...
sources: []
---
# HATHOR Research Paper 004 — Draft→Verified Knowledge Promotion Workflow
...
- `hath0r knowledge review` — interactive queue for the operator (lightweight editor per the boards).
- `hath0r knowledge review --list --json` — machine-readable queue for reporting.
...
- **Public tier** → not promotable by HATHOR; external material is ingested as reference with `status=external`, never `verified`.
...
- **Infra "draft/verified" concept from the Overview board — native HATHOR**, not an Infra import; noted for completeness.
