---
id: HATHOR-ARCH-001
title: HATHOR — Architecture & Mermaid Reference
...
sources: []
---
# HATHOR — Architecture & Mermaid Reference

- **Document ID:** HATHOR-ARCH-001
- **Status:** DRAFT v0 — visual companion to HATHOR-REQ-CORE-001
- **Date:** 2026-09-11
- **Author:** Oz (Agent), commissioned by Raymond Bayly (BaylyAI)
- **Companion to:** `hathor-req-core-001-initial-requirements-20260911.md`, plan `HATHOR-PLAN-001` (`hath0r-plan-001-platform-roadmap-20260913.md`)
...
- **Purpose:** Provide a single visual reference for the HATHOR core platform — planes, bot families, container taxonomy, request flow, hierarchy chain resolution, knowledge and ticketing lifecycles, universal project layout, validation fabric, orchestration gateway, and deployment topology.
...
| §8 Universal Project Layout | `.hath0r/` shape + AGENTS.md chain | UPL-001..005 |
...
```mermaid
flowchart LR
  H["Human operator<br/>(dev · DVO · curator)"] --> CLI["HATHOR CLI<br/><b>single control plane</b>"]
...
flowchart TB
  Root(["<project>/"])
  Root --> HathorMeta[".hath0r/<br/><i>hidden HATHOR metadata</i>"]
  HathorMeta --> Man["manifests/"]
  HathorMeta --> Rules["rules/"]
  HathorMeta --> State["state/"]
  State --> Runs["runs/"]
  State --> CL["checklists/"]
  State --> Spool["spool/  (telemetry)"]
  HathorMeta --> Know["knowledge/  (project tier)"]
  Root --> Cfg["cfg/"]
  Root --> Src["src/"]
  Root --> Lib["lib/"]
...
AGENTS.md resolves nearest-first as the CLI walks up from the working directory. `hath0r repo validate` enforces the shape; `hath0r repo init --from-infraos` converts legacy `.infraOS/` trees. [UPL-001..005]
...
flowchart LR
  ...
  CLIT -->|append JSONL| Spool[("Local append-only spool<br/>.hath0r/state/spool/")]
...
```mermaid
flowchart LR
  Q["Query"] --> P["Project tier<br/>.hath0r/knowledgebase/ + project MCP"]
  P -->|hit| Ans1["Result"]
...
```mermaid
flowchart LR
  Agent["Agent / worker bot"] --> CLI["HATHOR CLI"]
  H["Human"] --> CLI
...
flowchart LR
  ...
  Reg --> Run["HATHOR Container running<br/>(bot + CLI + contracts)"]
...
```mermaid
flowchart LR
  Dev["Developer machine<br/>Docker Compose<br/>MBI · spool · project .hath0r/"]
...
sequenceDiagram
  ...
  participant Obs as Observation-Bot
  participant Tower as Control Tower
  Dev->>CLI: hath0r process runbook run dvo-deploy-ticket --context pr_url=… --dry-run
  ...
  Dev->>CLI: hath0r process runbook run dvo-deploy-ticket … --yes
...
This is the concrete "Ticketing Plane → Container promotion" arrow enforced by HATHOR: builder + verifier (agent), operator (human). [TKT-011, CNT-007]
...
| Universal project shape | `.hath0r/` + AGENTS.md chain + `hath0r repo validate` | UPL-001..004 |
