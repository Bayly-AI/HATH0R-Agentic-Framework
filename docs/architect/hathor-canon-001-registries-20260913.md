...
Run-log events live in `.hath0r/state/runs/<run_id>/events.jsonl` and are *mirrored* to the spool as `orchestration.*` events; they are two views of one fact, deduped on `event_id`.
...
Language/product linters are project-registered packs (`.hath0r/rules/linters.yaml`, RP-007 §4.2) and are not enumerated here; org-distributed packs MUST be signed (RP-013).
...
1. **`HATHOR-` prefix = document; `AEG-` prefix = requirement.** `HATHOR-REQ-BOT-001` is a *document*; `AEG-REQ-BOT-001` is a *requirement in CORE-001*. This near-collision is grandfathered for existing docs; **new documents** use `HATHOR-<TYPE>-<NNN>` with `TYPE ∈ {ADR, RP, TS, PLAN, REQ, ARCH, CANON, GUIDE, REPORT, SESSION}` and MUST NOT mint a document ID whose body matches an existing requirement prefix.
2. **File naming (new docs):** `hath0r-<type>-<nnn>-<slug>-<yyyymmdd>.md`, lowercase. Existing files keep their names; INDEX.md carries the Doc-ID column for lookup.
