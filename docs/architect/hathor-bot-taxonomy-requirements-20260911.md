...
title: HATHOR — Bot Taxonomy, Anatomy & Default Command Contract
summary: 1. **Infra reuse only when better** — HATHOR may adopt Infra processes/architecture only where they demonstrably exceed a from-scratch HATHOR design. Every adoption is logged in §9.
doc_type: REQ
diataxis: reference
...
sources: []
---
# HATHOR — Bot Taxonomy, Anatomy & Default Command Contract
...
- **Sources:** HATHOR "CLI" board, HATHOR/HATHOR "Overview" board (Agentic Application Framework)

---

## 0. Canonical Rules Governing This Document

1. **Infra reuse only when better** — HATHOR may adopt Infra processes/architecture only where they demonstrably exceed a from-scratch HATHOR design. Every adoption is logged in §9.
2. **Research only** — HATHOR work is limited to reports and requirements. No implementation is authorized by this document.

---

## 1. Purpose & Scope

Define the canonical taxonomy (bot families and roles), anatomy (mandatory structural blocks), runtime behavior, knowledge/storage model, and the default command contract that **every** HATHOR bot MUST implement.
...
Every HATHOR capability MUST be delivered as a small, single-role bot ("microbot"). One bot, one responsibility.
**Acceptance criteria:**
...
- Every unit of work executed under HATHOR can be traced to a complete chain (Procedure through Checklist).
...
Bots MUST be reachable only through the HATHOR CLI. Agents, users, and bots MUST NOT invoke bots directly.
**Acceptance criteria:**
...
**Acceptance criteria:**
- `manifest` command emits the manifest; it validates against HATHOR manifest schema 1.1.0 (HATHOR-RP-001, as amended by RP-014).
...
Project = repo-local knowledge + project MCP (under the hidden `.hath0r/` folder of the Universal Project Layout). Machine = CLI knowledge + local vector server (hybrid keyword + vector). Organization = org MCP. Public = last resort.
**Acceptance criteria:**
...
**Acceptance criteria:** Output validates against the HATHOR manifest schema; equals what the CLI has registered.
...
| Infra monolithic CLI-service model | **Reject** | HATHOR microbot + single control plane supersedes it. |
...
1. **Manifest schema v1** → **HATHOR-RP-001** (`./hathor-rp-001-manifest-schema-v1-20260911.md`) — full JSON schema; 4-phase HELLO/OFFER/BIND/VERIFY handshake; capability-based routing; AEG-MAN-001..007.
2. **Registry model** → **HATHOR-RP-002** (`./hathor-rp-002-registry-discovery-20260911.md`) — hybrid: machine-local index (MBI) for all runtime routing, Tower registry (TBR) as authority; bounded offline trust; AEG-REG-001..006.
3. **Telemetry transport** → **HATHOR-RP-003** (`./hathor-rp-003-telemetry-transport-20260911.md`) — spool-and-drain (CLI append to local spool, Observation-Bot drains); event bus rejected at machine tier; AEG-TEL-001..007.
4. **Draft→verified promotion** → **HATHOR-RP-004** (`./hathor-rp-004-knowledge-promotion-20260911.md`) — status-on-record, queue-as-view; tiered reviewer authority; dispute/supersede/expiry lifecycle; AEG-KPW-001..008.
5. **Hierarchy consolidation** → **HATHOR-RP-005** (`./hathor-rp-005-hierarchy-consolidation-20260911.md`) — six identities, one chassis; consolidation rejected; fan-out chain resolution; AEG-HIE-001..006.
