...
- **Scope rule:** This specification describes *how* to build the system. It authorizes **no code by itself**; implementation requires an authorizing ticket per HATHOR governance (CORE AEG-REQ-TKT-004).
...
| **TS-D-003** | **Assumption ledger is event-sourced (append-only JSONL, state = fold of events).** | Matches HATHOR "status-on-record, queue-as-view" (CORE AEG-REQ-KNO-007); crash-safe; auditable. | No. |
...
| **TS-D-005** | **First build + dogfood target is the HATHOR greenfield repo** (new `hath0r` CLI). InfraAPI `make q-gates` is wrapped as an L3 subprocess backend, not modified. | User selection. | n/a |
...
Working module path in examples: `github.com/Bayly-AI/HATH0R-CLI`. Repository/module ownership MUST be frozen before M0; until then this string is illustrative rather than a canonical public import path, and any change must update TS-001/002/003 and PLAN-002 atomically.
...
The system ships inside the single `hath0r` binary. There is no always-on daemon in P0–P2 (watch mode is P4). Everything runs as short-lived CLI invocations, consistent with CORE AEG-REQ-BOT-009 (statelessness) and RP-003 (spool-and-drain, no machine-tier broker).
...
```text
hath0r/
  go.mod                      # working module path github.com/Bayly-AI/HATH0R-CLI; freeze at M0
  cmd/hath0r/main.go           # binary entrypoint; wires cobra
  internal/
    ...
    upl/                      # .hath0r/ layout resolution, run dir mgmt
...
All runtime state lives under the project `.hath0r/` (CORE §7). No state in the binary, no global mutable state outside `$XDG` machine tier.
...
Machine tier (cross-project): `${XDG_DATA_HOME:-~/.local/share}/hath0r/index/` holds the MBI-lite of installed linters/validators (RP-002 install-time indexing, AEG-REQ-REG-002).
...
| **`hath0r` command** | Canonical CORE §4.5 table (`0–7`). | AEG-REQ-PLAT-005 |

### 3.2 `hath0r validate *` exit mapping (`internal/exit`)
...
Language/product packs are external processes. HATHOR drives them uniformly:

**Invocation:** `argv` from `linters.yaml` template; a JSON request on **stdin**:

```json
{
  "schema": "hath0r.linter.input/1",
  "id": "eslint-northstar",
  "class": "js.sonar",
  "mode": "diff",
  "root": "/abs/project",
...
Bundled defaults (`packs/suites/`, `go:embed`) mirror RP-007 §7: `suite.change.fast|default`, `suite.claim.step-done|run-complete|pr-ready`, `suite.build.container`, `suite.knowledge.push`, `suite.health.project`. Project `.hath0r/rules/suites/` overrides by `id`.
...
## 7. `hath0r validate change` (P0, implementation grade)

### 7.1 Command

```text
hath0r validate change
...
- explicit `hath0r validate assumption recheck [--all|<id>]`
...
```text
hath0r validate assumption open  --class <c> --statement <s> [--ttl PT30M]
                                [--check-interval PT2M] [--evidence-query <json>] [--run <id>]
hath0r validate assumption list  [--status held|broken|expired|…] [-o json]
hath0r validate assumption recheck [<id> | --all | --stale-only]
hath0r validate assumption waive <id> --reason <r> [--expiry <ts>]   # human only
hath0r validate assumption close <id>
```
...
Even in-proc, each validator carries chassis metadata so it registers/selftests like any bot (CORE AEG-REQ-BOT-008): `manifest`, `contract`, `selftest`, `version`. `hath0r bots selftest` iterates validators + runs native-linter golden fixtures + subprocess-pack `--help/--dry-run` probes.
...
```text
hath0r validate record --class tests -- pytest -q   # renamed from `hath0r run --record` per ADR-003 §2.2
```
...
### 10.4 `val-claim-Bot` + `hath0r validate claim`

```text
hath0r validate claim <claim-type> [--tier <t>] [--run <id>]
```
...
|---|---|
| `hath0r delivery commit` | `validate change --diff staged` (suite.change.default) |
| `hath0r delivery pr prepare` | `validate claim pr.ready` |
| `hath0r knowledge push` | `suite.knowledge.push` (secrets/meta/microburst-size) |
| container build (`hath0r delivery build`) | `suite.build.container` |
| `hath0r process … run` finalize | `validate claim run.complete`; per-step `validation.suite` (P4) |
...
`validation.yaml: strict_undeclared: false` default. When true, a blocking finding whose `class` maps to a known assumption class with **no** open assumption in the run adds an `assumption.undeclared` finding and blocks. Default on only for HATHOR self-dogfood.

---

## 12. Telemetry (P0+, RP-003 conformance)

- `internal/telemetry` appends events to `.hath0r/state/spool/*.jsonl`; never networked here (Observation-Bot drains — out of scope).
...
1. `--flag` explicit
2. project `.hath0r/rules/*.yaml`
3. machine `${XDG_CONFIG_HOME}/hath0r/*.yaml`
4. embedded defaults (`go:embed`)

Per-bot config (RP-014 §3.6) resolves on its own nearest-first chain: `--flag` → project `.hath0r/rules/bots/<name>.yaml` → machine `${XDG_CONFIG_HOME}/hath0r/bots/<name>.yaml` → embedded defaults, validated against the bot manifest's `runtime.config_schema`.
...
- Assumption/evidence stores are already file-based under `.hath0r/` (a container mounts the run dir or drains via CLI).
...
- `hath0r validate linters install <ref>` indexes into MBI-lite; `ml-langpack-present` fails a repo that contains a language with no bound pack (AEG-VAL-010).
...
- `hath0r validate watch --run <id> --tier L0|L1`: ephemeral, per-run (open Q1 lean = ephemeral); `fsnotify` + git-index poll; runs L0/L1; appends findings; drives `assumption.tick`; `drain|stop` supported; no SoT writes.
...
| P2 | `hath0r validate record` (ADR-003 §2.2); `val-evidence-Bot`; claim engine + `claims.yaml`; four gates; façade wiring for commit/pr/knowledge/build/run-finalize; negative-path integration green |
...
6. **Monorepo run scoping:** one run per repo vs per package when multiple `.hath0r/` exist (ties to CORE §17 Q5 MBI scope).
