...
HATHOR already has a superb *detection* fabric (RP-007 CVS) and a *guard* fabric (CORE §14, ten mechanical gates). What it does **not** have is a **conductor**: a system-native owner of control flow that guarantees, for any unit of work, that the full required set of bots, linters, and validators executes — in order, with no skips — regardless of what the agent chooses to do.

Two doctrine statements expose the gap:

- **RP-007 §6.1** makes validation the *agent's* responsibility ("Agents MUST … run `hath0r validate change` … run `hath0r validate claim` …"). This is **voluntary compliance**.
...
- reuses existing HATHOR components (Proctor, Process-Bot, hierarchy assets, CVS suites/claims/assumptions, spool-and-drain telemetry, Checklist, Tower) rather than inventing parallel machinery.
...
|---|---|
| `PLAT-001` single control plane, no secondary surface | AOG adds **no** new control surface; reached via `hath0r process`/`hath0r proctor`. Events enter through CLI-mediated hooks/façades only. |
...
|---|---|---|
| `run.open` | `hath0r process run open` / façade | Instantiate run from graph; mark entry nodes admissible |
| `fs.write` / `git.stage` | CLI hook, `validate watch` | Bind to active node; run node's `on_change` suite (L0/L1) |
| `claim.step.done` | agent / `hath0r process node complete` | Evaluate node exit criteria (suites + evidence + assumptions) |
...
| `run.finalize` | `hath0r process run finalize` / delivery façade | Invoke completeness reconciler (§7.4) |
...
```text
<project>/.hath0r/state/runs/<run_id>/
...
- **Two execution modes, identical semantics** (because state is durable):
  ...
  - *Resident mode* (P4 opt-in): a drain loop (`hath0r process conduct --run <id>`) tails the log; `drain|stop` supported like Observation-Bot.
...
- A killed run **resumes from the folded state**: `hath0r process run resume <id>` recomputes admissible nodes and continues. Incomplete nodes re-run; `completed` nodes do not.
...
Façade wiring (extends TS-001 §11.2): `hath0r delivery commit|pr prepare`, `knowledge push`, container build, and `process run finalize` all call `proctor.Guard` → which now includes the Sequence/Barrier Gate + reconciler where applicable.
...
Every gateway refusal MUST return the next-required unit(s) via the standard envelope + `hath0r proctor gateway explain`.
...
**AC:** `hath0r --help` renders one screen; new commands nest under `process`/`proctor`.
...
4. **Cross-run / cross-repo graphs.** Monorepo with multiple `.hath0r/` — one run per repo vs per package (ties to CORE §17 Q5, TS-001 Q6).
