---
id: HATHOR-RP-006
title: 'HATHOR Research Paper 006 — The Ticketing Plane: Work as a Source of Truth'
...
sources: []
---
# HATHOR Research Paper 006 — The Ticketing Plane: Work as a Source of Truth
...
- **Related:** HATHOR-RP-002 (hybrid authority + bounded offline trust), HATHOR-RP-003 (spool-and-drain transport), HATHOR-RP-004 (status-on-record, queue-as-view), `hathor-containerization-article-20260911.md` (the DVO promotion arrow)
...
- **Sources:** HATHOR "CLI" and "Overview" boards; InfraOS governance (cr-jira-ticket-001/002/003, cr-deploy-gov-001, cr-bug-intake-001, cr-branch-gov-001, cr-003/cr-004 connection-first, cr-005/cr-auth-token-001 secrets order, `jira-standards.md` estimation)
...
HATHOR already decomposes execution into a hierarchy chain (Procedure → … → Checklist, RUN-001). The ticket does not replace that chain; it *binds* to it:
...
```json
{
  ...
  "canonical_id": "HATHOR-WORK-8f2c9e1a",
  "provider": { "kind": "jira", "ref": "AMD-1234", "url": "https://…/browse/AMD-1234", "project": "AMD" },
  "type": "task",
  "title": "…",
  "description": "…",
  "status": "in_progress",
  "epic": { "canonical_id": "HATHOR-WORK-1a7b…", "provider_ref": "AMD-340", "required": true },
...
- **`canonical_id`** is HATHOR-owned and stable across providers; **`provider.ref`** is the provider-native key (may be absent while a ticket is *buffered* in the backup plane, §5).
...
```mermaid
flowchart LR
  Agent["Agent / worker bot"] --> CLI["HATHOR CLI<br/>(sole control plane)"]
...
The plane is where the ticket-based-economy rules stop being prose and become mechanical gates. These are HATHOR-native restatements of cr-jira-ticket-001/002/003 and cr-deploy-gov-001.
...
`hath0r work list|show` exposes canonical tickets (identity, provider ref, status, epic, links, sync state) in JSON and human form.
...
1. **Canonical work-id scheme** — opaque UUID (`HATHOR-WORK-<uuid>`) vs. provider-key-prefixed. UUID decouples from providers but costs human readability during a buffered outage.
