...
| `val-drift-Bot` | after M-A dogfood begins | drift detection is valuable once a real repo lives under HATHOR rules | S/M |
...
**Deliverable:** `hath0r-agent-skill.md` (~800 tokens), versioned with the CLI, generated-and-verified against `hath0r schema` in CI so it cannot drift.
...
**Stories:** SKL-S1 author pack (S); SKL-S2 CI check — pack claims validate against `hath0r schema` output (S); SKL-S3 token-budget bench ≤ 800 tokens (S). Schedule: with PLAN-002 M1 (the commands it documents must exist).
