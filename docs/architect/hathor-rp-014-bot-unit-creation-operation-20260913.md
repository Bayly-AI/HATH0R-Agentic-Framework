...
- **Amendments (ratified + applied 2026-09-13):** RP-001 §2 (manifest 1.1.0 `governance` triad + `runtime.requires_capabilities`/`config_schema`/`executor_kind`); BOT-001 `AEG-BOT-CMD-011/013/018`; CANON-001 §4/§5/§8; ADR-003 §2.2 surface note (`hath0r tower bots init`); RP-007 §4.1; TS-001 §2.3/§13 — see §11 and `PENDING-EDITS.md` §2 (D8).
...
|---|---|---|---|
| I1 | **Definition** | Own manifest, own governance bundle, own UUID, own semver — even when six identities share one chassis (`AEG-HIE-001..003`) | `hath0r tower bots show <name>` resolves a complete bundle with no reference to a sibling |
...
|---|---|---|---|
| **Registration** | present; required sections; ≤ budget; digest matches | schema-valid; constraint-only vocabulary; references declared commands; fixtures present; digest matches | bot-carried `hath0r-principles` set referenced in full; bot principles ≤ 5, each traces to a carried principle; digest matches |
...
<bot-root>/
  ...
  principles.yaml        # bot-carried hath0r-principles ref + ≤ 5 bot principles
...
At install, the CLI copies the verified bundle to `<project>/.hath0r/manifests/<name>/` (UPL) and indexes it into the MBI (`AEG-REQ-REG-002`).
...
```json
{
  ...
  "governance": {
    ...
    "principles": { "path": "principles.yaml", "digest": "sha256:…", "platform_set": "hath0r-principles@1" }
  }
}
```
...
2. **Project rules** (`.hath0r/rules/*.yaml`) — validation profiles, linter packs, claims, suites, graphs, knowledge thresholds.
...
rules:
      ...
      schema:
        properties:
          effects:
            properties:
              paths_written:
                items: { pattern: "^\\.hath0r/state/checklists/" }
    on_violation: refuse                     # refuse | degrade  (no grant vocabulary exists)
    message: "checklist-Bot wrote outside .hath0r/state/checklists/"
  ...
  - id: cl-r-002
    ...
    remediation: "Open a run (hath0r process run open) or dispatch through the conductor."
```
...
#### 3.5.1 The bot-carried principle set (`hath0r-principles@1`, inherited, non-removable)
...
version: 1
bot: checklist-Bot
platform_set: hath0r-principles@1        # schema field naming retained; bot-carried set inherited in full
...
Config tunes; it never governs. It resolves nearest-first exactly as TS-001 §13: `--flag` → project `.hath0r/rules/bots/<name>.yaml` → machine `${XDG_CONFIG_HOME}/hath0r/bots/<name>.yaml` → embedded defaults. It is validated against `runtime.config_schema`, contains connection *names* only (never credentials), and is excluded from the bundle digest by design — an operator may retune a bot without re-signing it, and cannot loosen a rule by doing so.
...
flowchart LR
  ...
  subgraph RunScoped["Run-scoped — durable, event-sourced, platform-owned"]
    RL["Run ledgers<br/>.hath0r/state/runs/RUN_ID/*.jsonl"]
    CA["Cache<br/>.hath0r/state/cache/SUBSYSTEM/ · digest-keyed · disposable"]
  end
  subgraph Durable["Durable & shared — sources of truth or record"]
    PS["Project state<br/>.hath0r/state/checklists/ · .hath0r/manifests/"]
    KN["Knowledge Plane<br/>.hath0r/knowledgebase/ · machine · org MCP"]
...
| **Run ledgers** | one run | `.hath0r/state/runs/<run_id>/{events,findings,evidence,assumptions}.jsonl` | Process-Bot (`events.jsonl` transitions); the emitting bot for findings / evidence / assumptions | append-only, via CLI, `flock` + `O_APPEND` (TS-001 §8.2) | state = fold; dedupe on `event_id`; torn trailing line tolerated |
| **Cache** | project or machine | `.hath0r/state/cache/<subsystem>/`, `${XDG_DATA_HOME:-~/.local/share}/hath0r/` | bot / chassis | yes | digest-keyed; delete and recompute; **never authoritative** |
| **Project state** | project | `.hath0r/state/checklists/`, `.hath0r/manifests/` (UPL) | platform | only with `side_effects: local` and a scoping rule | git; re-derive from ledgers |
...
- **reads** state by folding (or via `hath0r process run status -o json`) into working memory,
...
2. **Scaffold:** `hath0r tower bots init <role>-Bot --family <orchestration|hierarchy|observation> [--tier <t>] [--role validator]` (proposed, §9) writes the bundle from the family template: manifest skeleton (UUID minted, name normalized per CANON-001 §7.3), directive skeleton with the seven sections, family default rules with fixtures, `principles.yaml` referencing `hath0r-principles@1`, and default-command stubs.
...
Operational surface for humans and agents: `hath0r tower bots list|show <name> [--governance]|selftest <name>`, `hath0r proctor gateway explain` for refusals, `hath0r doctor` for fabric health.
...
| **Hierarchy (six, one chassis)** | no | knowledge reads; own-tier records; `checklist-Bot` writes `.hath0r/state/checklists/` | writes confined to own tier; `hierarchy_chain` on every write; partial-chain honesty | P02, P05, P08 |
...
{
  "manifest_version": "1.1.0",
  "identity": { "uuid": "…", "name": "checklist-Bot", "family": "hierarchy", "tier": "checklist", "version": "1.0.0", "owner": "hath0r-core",
    "provenance": { "builder": "hath0r-forge", "source_repo": "hath0r/hierarchy-chassis", "source_commit": "…", "built_at": "…" } },
...
> **Success.** One invocation succeeds when the ledger reflects exactly the node states the run log proves, and the write landed under `.hath0r/state/checklists/`.
...
Every bot MUST reference `hath0r-principles@1` in full; bot-specific principles MUST number ≤ 5, MUST each cite ≥ 1 carried principle, and rank below the carried set.
...
**AC:** Static check finds no bot-owned path outside `.hath0r/state/cache/<…>` and declared scopes; kill-at-any-instant test loses only working memory.
...
`hath0r tower bots init` MUST produce a bundle that passes `selftest` unmodified (with the stub executor) and includes the family default rules.
...
| ADR-003 §2.2 (surface note) | `hath0r tower bots init` — additive verb under `tower`; domain count unchanged; no decision change | applied 2026-09-13 (D8) |
| TS-001 §2.3 / §13 | per-bot config paths `.hath0r/rules/bots/<name>.yaml` and `${XDG_CONFIG_HOME}/hath0r/bots/<name>.yaml` | applied 2026-09-13 (D8) |
...
4. The twelve bot-carried principles (`hath0r-principles@1`) are accepted as the canonical set and `report.decisions[]` as their observability mechanism.
