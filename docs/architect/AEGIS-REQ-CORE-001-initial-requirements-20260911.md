...
### 0.1 What AEGIS is
AEGIS is an **agentic governance platform** — a code-agnostic, agent-agnostic control plane that turns humans + AI agents into first-class, accountable actors against any codebase or operational surface. It is the concrete realization of the HATHOR Agentic Application Framework's control-plane pillar.

### 0.2 What AEGIS is not
...
- Not a rewrite of `infraos-os`. AEGIS is a **greenfield core platform**; specific InfraOS assets are carried forward per §12.
...
4. **Infra reuse only when better.** An InfraOS component is adopted only when it *demonstrably exceeds* a from-scratch AEGIS design. Every adoption is logged (§12).
...
- One CLI binary (`aegis`) with ≤ 9 top-level domains + always-on meta commands (ADR-003 §2.2).
...
- Universal Project Layout (`.aegis/`) recognizable in any repository — code-agnostic.
...
### 1.1 In scope for v1
- Core CLI (`aegis`) — command tree, global contract, exit-code contract, schema introspection.
...
- Universal Project Layout tooling (`aegis repo init|validate`).
...
- Multi-tenant SaaS operation. AEGIS v1 is per-org, self-hostable.
...
The platform MUST expose exactly one operational entrypoint (`aegis`). Every agent, human, or bot capability is reached through it.
...
**AC:** `aegis planes list` enumerates the three; each plane has a designated authority and a bounded-degradation policy.
...
**AC:** `aegis doctor exit-codes` prints the table; conformance test verifies every command in the manifest.

### AEG-REQ-PLAT-006 — Universal Project Layout
Every AEGIS project MUST carry a canonical layout (§7). Bots and CLI logic MUST treat every project identically once the layout is validated.
**AC:** `aegis repo validate` returns exit 0 for a compliant project; scoped `AGENTS.md` chain resolves cleanly from root down.
...
**AC:** `aegis --help` renders one screen; `aegis --help --include-legacy` shows aliases.

### AEG-REQ-PLAT-009 — Machine-readable schema introspection
The CLI MUST expose a bounded schema surface (`aegis schema` with `--domain` and pagination). Full unfiltered dump is opt-in only. `[Spike 04, ADR-001]`
...
Nine domains per ADR-003 §2.2: `validate` is promoted (highest agent-traffic surface); registry introspection folds under `tower bots …` with `aegis bots …` retained as a hidden alias. Compatibility aliases from InfraOS are hidden behind `--include-legacy` (see §12).
...
`aegis help`, `aegis <cmd> --help`, and `aegis <cmd> help` MUST all work. `[CLI-Research §2.3.2]`
...
**Exit-code boundaries (ADR-003 §2.3).** The table above governs the `aegis` CLI boundary only. The CR-017 bot boundary (`0|1|2`, where 2 = degraded) applies to bot executables and is translated by the chassis: a bot-boundary `2` surfaces as `{degraded: true}` in the response envelope, never as CLI exit 2. Where a distinct process exit is operationally required for a data state, it MUST be declared as a CLI Spec v0.3 `outcomes[]` entry with a code overlapping no error code. Legacy corpus phrasing "degraded (exit 2)" reads accordingly.

**CLI Spec envelope mapping.** The AEGIS error envelope `{code, message, remediation, provenance, ttl}` maps onto the CLI Spec error envelope as `code → error.kind`, `message → error.message`, `remediation → error.hint`, `{provenance, ttl} → error.details`; `aegis schema` declares the full `errors[]` (and `outcomes[]`) tables so both contracts hold simultaneously.
...
`aegis bots list [--json]` MUST expose the MBI (identity, capabilities, state, digests) sufficient to diagnose any routing refusal without Tower access. `[AEG-REG-006]`
...
```text
<project>/
  .aegis/           # hidden AEGIS metadata (manifests, rules, run state, checklists)
    manifests/
    rules/
    state/
      runs/
  ...
  Makefile          # OPTIONAL thin façade over stable aegis CLI contracts
```

### AEG-REQ-UPL-001 — Layout as contract
`aegis repo validate` MUST succeed only for a project matching the canonical layout (case-sensitive paths). Missing `.aegis/` fails validation. `[CLI-Research §1.7]`
...
**AC:** `aegis repo agents list` prints the resolved chain for any path.

### AEG-REQ-UPL-003 — Makefile as thin façade
Any Makefile in an AEGIS project SHOULD invoke only stable `aegis` CLI contracts. `bin/` MUST NOT contain a parallel control plane. `[CLI-Research §2.2, §5-E]`
**AC:** `aegis repo validate --deep` warns when `bin/` contains executables not registered as CLI aliases.

### AEG-REQ-UPL-004 — Git-first
Every AEGIS project MUST be a Git repository. Non-git projects are outside v1 scope.

### AEG-REQ-UPL-005 — Migration from `.infraOS`
`aegis repo init --from-infraos` MUST convert a legacy `.infraOS/` tree into `.aegis/`, preserving runbooks, checklists, workflows, and rules; original tree retained until operator confirms.
**AC:** Post-migration `aegis repo validate` passes; a diff report is produced for operator review.
...
- **Project** = repo-local `.aegis/knowledge/` + project MCP
...
`aegis work status` MUST report authoritative vs buffered counts, epic-compliance, and reconcile health. `[AEG-TKT-013]`
...
Every AEGIS container MUST carry the seven anatomical blocks (Identity, Manifest, Contract, Executor, Knowledge interface, Connection interface, Telemetry surface) at the deployment layer.
...
Containers MUST contain zero baked credentials. Runtime injection only (`aegis secrets exec` locally; IRSA or EKS Pod Identity + secret mounts on EKS).
...
Every InfraOS carry-over decision is logged. Adopt = brought into AEGIS unchanged in spirit; Adopt (shaped) = restated to fit AEGIS taxonomy; Reject = not carried forward.

| Source | Decision | Rationale | Landing requirement |
|---|---|---|---|
| Single-binary CLI entrypoint (AD-001, `infraos-os`) | **Adopt** | Already an AEGIS-class ambition; proven ergonomics. | AEG-REQ-PLAT-001 |
...
| Quality/release gates + AGENTS contracts (AD-008) | **Adopt** | Feeds Proctor-Bot gate surface. | `aegis proctor gate` |
| Product `.infraOS/` per-repo state | **Adopt (shaped) → `.aegis/`** | Same idea, canonicalized layout + migration tool. | AEG-REQ-UPL-005 |
...
| cr-jira-ticket-001/002/003 (ticket economy, epic, PR bind) | **Adopt (native)** | Native to AEGIS ticketing plane. | AEG-REQ-TKT-004..006 |
...
| cr-aws-gov-001 EKS-only rule | **Adopt** | AEGIS containers deployed on EKS only in AWS environments. | Deployment doctrine |
...
Cheap commands (`aegis version`, `aegis health`, per-bot `health`) MUST complete in < 250 ms wall clock in steady state. `[BOT-CMD-010]`

### AEG-REQ-NFR-002 — Agent-orientation token budget
The default orientation pack (`aegis version`, `aegis proctor preflight agent`, `aegis process orient`, `aegis operator connections list`, `aegis operator mcp-servers list`, `aegis process workflow list`) MUST fit in ≤ 2500 output tokens. `[Report 05 §7]`

### AEG-REQ-NFR-003 — Schema payload budget
`aegis schema --domain <x>` default page MUST be ≤ 8 KB. `[Spike 04 §3.4]`
...
AEGIS v1 MUST support macOS and Linux (darwin/linux, arm64+amd64); Windows is out of scope for v1 (ADR-004 §2.4). POSIX facilities (`flock`, TTY semantics, XDG paths) MAY be relied on without shims.

---

## 16. Validation Criteria (v1 acceptance)

An AEGIS v1 release is accepted when *all* of the following pass:
...
1. **CLI binary name — resolved.** `aegis` is the greenfield binary of record (ADR-003 §2.1).
...
5. **Cross-project MBI scope.** Machine-level vs project-workspace-level when multiple AEGIS projects coexist on one host. `[RP-002 Q2]`
6. **Backup TS implementation choice — resolved.** Build the minimal greenfield `aegis-backup-ts` (accepted ADR-005).
...
8. **AWS deployment target parity.** EKS-only per InfraOS carry-over — confirm for AEGIS v1 or allow ECS-Fargate as an alternative Class-A hosting model.
...
*Draft v0 — research-informed platform requirements for the greenfield AEGIS core. No implementation authorized by this document. Review and amend before promotion to `verified`.*
