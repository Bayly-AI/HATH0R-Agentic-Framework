...
2. **Produces the initial launch roster** (§4) in the same table shape as the Micro-Linter catalog (`hath0r-rp-007` §4.1): **Group · Name · What it does · Drift it stops** — with a **Launch** column added, since this is a phased launch list rather than an always-on set.
...
| `hath0r-ts-001` / `hath0r-plan-002` / `hath0r-plan-003` | Where validator phases and the remaining-validator placements are already scheduled; everything else here is this paper's own proposal |
...
Same pattern as the Infra adoption logs (`hath0r-bot-taxonomy-requirements` §9, `hath0r-rp-007` §11), applied to the outline image instead of Infra.
...
| 2 | Cleaner Service | **Gap — deferred pending ADR** | No canonical owner for `.hath0r/state/` retention/purge (run dirs, spool quota, expired drafts). Candidate homes: a Process-Bot subcommand, or a new Proctor-owned maintenance-Bot justified the same way RP-007 justified Validator-Bots. Not decided here. |
| 3 | Tower Service | **Correction — not a bot** | Control Tower is the authenticated platform authority, explicitly excluded from the bot roster and not a universal terminal hop. `[AEG-REQ-BOT-006, RP-010 §2]` Reached directly via authenticated `hath0r tower …` commands. |
| 4 | Init Service | **Fold into existing capability** | `hath0r repo init\|validate` (UPL bootstrap) is a Process-Bot-mediated CLI capability, not a standalone bot. |
...
**Phase sourcing legend:** *Sourced* = placement is fixed by `hath0r-ts-001`/`hath0r-ts-002` or `hath0r-plan-002`/`hath0r-plan-003`; *Proposed* = this paper's own recommended sequencing where no approved roadmap covers base-chassis rollout.
...
| Validation · Proctor-owned | `val-assumption-Bot` | Ledger CRUD, TTL tracking, recheck scheduling, human-only waive | Undeclared load-bearing assumptions acted on as fact | P1 — *Sourced* (`hath0r-plan-002` CVS-P1-E2) |
...
| Validation · Proctor-owned | `val-ticket-Bot` | Mid-run re-check of no-ticket / epic / estimation / PR-bind / deploy prechecks | Ticket losing its epic mid-run without breaking dependent claims | With WS5 Ticketing after the Jira adapter — *Sourced* (`hath0r-plan-003` §2) |
| Validation · Proctor-owned | `val-hierarchy-Bot` | Chain completeness; checklist↔runbook sync; stale chain digest | Orphaned checklists; broken runbook/workflow/checklist links | With AOG P3 graph validation — *Sourced* (`hath0r-plan-003` §2) |
| Validation · Proctor-owned | `val-knowledge-Bot` | Draft/verified honesty; TTL flags; redaction preflight | Secrets/PII reaching a knowledge microburst | With WS6 Knowledge — *Sourced* (`hath0r-plan-003` §2) |
| Validation · Proctor-owned | `val-claim-Bot` | Maps claim type → required suite tier; verifies evidence exists | `pr.ready`-class claims asserted after only L1 (`CLAIM_TIER_INSUFFICIENT`) | P2 — *Sourced* (`hath0r-plan-002` CVS-P2-E2) |
| Validation · Proctor-owned | `val-drift-Bot` | Repo layout drift; Makefile façade violations; legacy path usage | Parallel control plane growing in `bin/` unnoticed | After M-A dogfood begins — *Sourced* (`hath0r-plan-003` §2) |
| Validation · Proctor-owned | `val-evidence-Bot` | Attests cited commands actually ran (exit codes, report digests in ledger) | Completion narrative with no recorded command evidence | P2 — *Sourced* (`hath0r-plan-002` CVS-P2-E1) |
| Validation · Proctor-owned | `val-completeness-Bot` | Reconciles graph-derived required units against the authoritative run/evidence ledgers with telemetry correlation at finalize | Mandatory nodes or suites omitted from a conducted run | AOG P2 — *Sourced* (`hath0r-plan-003` AOG-P2-E1; TS-002 §9.1) |
...
| 5 | `val-diff-Bot`, `val-contract-Bot` (mechanism only) | CVS P0 change-time path (`hath0r-ts-001` §5, §9) |
| 6 | UPL init/validate capability (Process-Bot-mediated, §3 row 4) | Universal Project Layout is a precondition for every other bot's `.hath0r/` state |
...
1. **Cleaner/maintenance ownership** (§3 row 2). Does `.hath0r/state/` retention become a `process-Bot` subcommand, or a new Proctor-owned maintenance-Bot roster member (RP-007-style justification)? Lean: subcommand first; promote to a roster member only if it needs its own failure domain. Not decided here.
...
Where an implementation phase is already fixed by `hath0r-ts-001`/`hath0r-plan-002`, this paper MUST cite it; where none exists, the row MUST be marked "unscheduled" or "proposed," never given an invented number.
...
| §4 launch roster | BOT-001 §2 rosters; RP-007 §5 validator roster; `hath0r-plan-002` §4 for sourced phases |
...
3. Validator phase citations (§4.2) are verified against `hath0r-plan-002`, `hath0r-plan-003`, `hath0r-ts-001`, and `hath0r-ts-002`; any still-unscheduled scope is explicit rather than silently dropped.
