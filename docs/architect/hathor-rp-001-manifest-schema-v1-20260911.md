---
id: HATHOR-RP-001
title: HATHOR Research Paper 001 — Bot Manifest Schema v1 & Contract-Negotiation Handshake
...
sources: []
---
# HATHOR Research Paper 001 — Bot Manifest Schema v1 & Contract-Negotiation Handshake
...
```json
{
  ...
  "owner": "hath0r-core",
  "provenance": {
    "builder": "hath0r-forge",
    "source_repo": "hath0r/hierarchy-chassis",
...
The governance triad (directive, rules, principles) is specified in **HATHOR-RP-014 §3**. Its three file digests are manifest fields, so the external DSSE envelope over the JCS-canonical manifest (RP-013 §6) covers the manifest and governance triad; editing any governance file changes the manifest digest (`MANIFEST_DIGEST_STALE`). Executor-byte coverage is excluded pending R10. `rules.count` equals the rule-set size; `platform_set` is `hath0r-principles@1`. `governance` is **mandatory**—a manifest without it is refused (RP-014 §3.2).
...
- **JSON Schema 2020-12 as validation vocabulary — Adopt (industry standard).** Exceeds inventing a HATHOR-only schema language.
