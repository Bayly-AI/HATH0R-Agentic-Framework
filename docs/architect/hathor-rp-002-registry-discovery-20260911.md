---
id: HATHOR-RP-002
title: HATHOR Research Paper 002 — Bot Registry & Discovery Model
...
sources: []
---
# HATHOR Research Paper 002 — Bot Registry & Discovery Model
...
1. **Machine Bot Index (MBI)** — built and owned by the CLI. Created at bot install/update time (not by scanning at request time). Maps `capability → {bot_uuid, manifest_digest, verification_state, verified_at}`. Stored machine-local; queryable via `hath0r bots list`.
...
`hath0r bots list` exposes the MBI (identity, capabilities, state, digests) in JSON and human form.
...
2. Whether the MBI should be per-machine or per-project-workspace when multiple HATHOR projects coexist on one machine (boards' Universal Project Layout suggests machine-level with project overlays).
