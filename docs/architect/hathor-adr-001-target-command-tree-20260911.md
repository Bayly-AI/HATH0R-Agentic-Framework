...
**Superseded in part (2026-09-13):** HATHOR-ADR-003 supersedes the `infraos-os`-rooted binary strategy and the "Greenfield HATHOR binary — rejected" alternative below. The platform of record is the greenfield `hath0r` binary; this ADR's surface is retained as the InfraOS migration/façade path. The domain taxonomy, lifecycle rule, global flags, and exit-code table remain authoritative.  
**Date:** 2026-09-11  
**ADR ID:** HATHOR-ADR-001  
**Relates to:** InfraOS AD-001 (single entrypoint), AD-002 (plugin registry), AD-005 (connections vs MCP), baseline HATHOR CLI research report  
...
```
Keep existing `repo refresh|canonical-verify|…` behavior behind this domain (AD-00R). Migration from `.infraOS` to the outline's `.ai/hath0r` target is a separate compatibility decision. *(2026-09-13: the canonical target is `.hath0r/` per CORE-001 §7.1 and ADR-004 §2.3; `.ai/hath0r` was the outline-era label.)*
...
- Matches HATHOR diagram language without discarding InfraOS services.
...
|------------|-------------------------|
| Greenfield HATHOR binary | ~~Splits control plane; violates AD-001 spirit~~ **Superseded by HATHOR-ADR-003 (2026-09-13):** greenfield `hath0r` adopted; this façade becomes the InfraOS migration path |
...
## References
- hathor-cli-research-report-2026-09-11.md  
