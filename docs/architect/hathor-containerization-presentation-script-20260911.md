---
id: HATHOR-GUIDE-002
title: HATHOR Containerization — Presentation Script
summary: flowchart LR Repo["Repo + micro-bots"] --> Lint["Micro-linters<br/>build-time Q-Gate"] Lint --> Build["Image build<br/>CVS metadata + labels"] Build --> C["HATHOR Container<br/>bot + CLI + contracts"] C --> Col["OTel C...
doc_type: GUIDE
...
sources: []
---
# HATHOR Containerization — Presentation Script
...
**Date:** 2026-09-11
**Source:** HATHOR CLI/Agent + HATHOR Overview architecture documents; InfraOS governance (cr-cvs-001, cr-docker-ports-001, cr-011, cr-aws-gov-001, cr-deploy-gov-001, cr-observability-001, cr-kb-push-001)
**Status:** DRAFT — session knowledge, pending Control Tower KB promotion

---

## SLIDE 1 — Title & Framing
*(Visual: HATHOR shield + container icon side by side)*

> "Today we're answering one question: when HATHOR meets containers, who owns what?
> We've spent months defining the Micro-Bot — the smallest unit of capability. Now we define its shell: the HATHOR Container — the smallest unit of deployment. And we'll define *our* role in that story, because based on everything we've learned from InfraOS, it's narrower and more powerful than 'we run containers.'"
...
> An HATHOR Container is the deployment quantum: the shell that gives one or more bots a runtime identity, a network address, and a promotion path.
...
> "InfraOS governance already answered this for us, and HATHOR inherits it.
...
> In HATHOR terms: the agent is a Process-level actor for containers. We compose and validate. The Tower and DVO own promotion and fleet state."
...
> If a container in our fleet can't satisfy every clause of that sentence, it isn't a HATHOR Container yet — it's a remediation item."
...
> One — ratify this as a canonical rule, working title `cr-hath0r-container-001`, and promote this document to the Control Tower knowledgebase.
...
> The Micro-Bot gave us a unit of capability we can trust. The HATHOR Container gives us a unit of deployment we can trust. The linters and the Tower make sure we never have to take either on faith. Thank you — questions?"
...
A: `InfraControl/cfg/gen3-port-registry.yaml` exclusively (InfraOS org-central path; in HATHOR projects the canonical file is `cfg/port-registry.yaml` per ADR-004 §2.1). Running-container reality is ground truth for diagnosis; registry drift is fixed in the repo, never worked around locally.
